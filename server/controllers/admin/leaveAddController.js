...
const addLeaves = async (req, res) => {
  try {
    ...
    const currentDate = new Date();
    const currentYear = currentDate.getFullYear();
    const march31 = new Date(currentYear, 2, 31); // March 31 of the current year
    const nextMarch31 = new Date(currentYear + 1, 2, 31); // March 31 of the next year
    const isMarch31 = currentDate.getMonth() === 2 && currentDate.getDate() === 31; // Check if today is March 31st

    const calculateLeaves = (probationEndDate) => {
      const monthsUntilMarch = Math.max(
        0,
        (nextMarch31 - probationEndDate) / (1000 * 60 * 60 * 24 * 30) // Approximate months
      );
      return Math.min(monthsUntilMarch * 1.5, 18); // Max 18 leaves
    };
    ...
    await Promise.all(
      employees.map(async (employee) => {
        ...
        const dateOfJoining = new Date(employee.dateofjoining);
        const probationEndDate = new Date(dateOfJoining);
        probationEndDate.setMonth(probationEndDate.getMonth() + 6); // 6 months probation

        let totalLeaves = 0;

        if (probationEndDate <= currentDate) {
          // Probation completed
          if (probationEndDate < march31) {
            totalLeaves = 18; // Completed probation before the financial year end
          } else {
            totalLeaves = calculateLeaves(probationEndDate);
          }
          totalLeaves = roundToNearestHalf(totalLeaves);
        ...
        if (leaveRecord) {
          if (!isMarch31) {
            console.log(`Skipping update for Employee ID: ${ename} as it's not March 31.`);
            return;
          }
          leaveRecord.leaves.total = totalLeaves;
          leaveRecord.leaves.available = totalLeaves - (leaveRecord.leaves.consume || 0);
          await leaveRecord.save();
        } else if (isMarch31) {
          const newLeaveRecord = new leavebalance({
            employee_id,
        ...
        console.log(`Updated leaves for Employee ID: ${ename}, Total Leaves: ${totalLeaves}`);
      })
    );
    ...
    if (res) {
      res.status(200).json({
        success: true,
        msg: "Leave records processed successfully.",
      });
    }
  } catch (error) {
    console.error("Error processing leaves:", error);
